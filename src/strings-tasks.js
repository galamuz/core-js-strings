...
function removeFirstOccurrences(str, value) {
  ...
  return firstIndex < 0
    ? str
    : str.substring(0, firstIndex) + str.substring(secondIndex);
}
...
function isPalindrome(str) {
  const noneSpaceStr = str.toLowerCase().replace(/[^a-zA-Z0-9]/g, '');
  const palindrome = noneSpaceStr.split('').reverse().join('');
  return palindrome === noneSpaceStr;
}
...
function reverseWords(str) {
  ...
  const reversedWords = words.map(function reverseStr(word) {
